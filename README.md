...
1. **Только backend** — скачать бинарник + `frontend-*.tar.gz`, распаковать, запустить `./reconciler -static ./frontend/`. Или просто запустить бинарник и открыть `localhost:8080` (API-only).
2. **Backend + Frontend** — раздельное развёртывание (backend на сервере, frontend через nginx).
3. **Desktop** — Electron-приложение. При первом запуске автоматически скачивает backend и frontend из GitHub Releases. Проверяет обновления при каждом запуске.
...
**Desktop (Electron, thin launcher — скачивает backend/frontend при первом запуске):**
- `.AppImage` + `.tar.gz` (Linux)
- `.dmg` (macOS arm64)
- `.exe` NSIS installer (Windows)

Desktop-версия проверяет обновления при запуске и предлагает обновиться.
